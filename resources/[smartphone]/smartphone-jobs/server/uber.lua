...
local function getName(user_id)
  local identity = API.getIdentity(user_id) or {}
  return identity.name or identity.nome or '(Erro)'
end

local function getPhone(user_id)
  local identity = API.getIdentity(user_id) or {}
  return identity.phone or identity.phone_number or identity.telefone
end
