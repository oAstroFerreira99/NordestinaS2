		...
		if Wanted[Passport] > os.time() then
			...
			if Call[Passport] <= os.time() and source > 0 then
				...
				TriggerClientEvent("Notify", source, "amarelo",
					"Você foi denunciado, parece que suas digitais estão no banco de dados do governo como procurado.",
					"Atenção", 10000)
				...
				for Passports, Sources in pairs(Service) do
					async(function()
						TriggerClientEvent("NotifyPush", Sources,
							{ code = 20, title = "Digitais Encontrada", x = Coords["x"], y = Coords["y"],
								z = Coords["z"], criminal = "Alerta de procurado",
								time = "Recebido às " .. os.date("%H:%M"), blipColor = 16 })
					end)
				end
			end
