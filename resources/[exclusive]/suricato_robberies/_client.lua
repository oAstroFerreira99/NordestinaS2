...
Citizen.CreateThreadNow(function() 
    ...
    if IsDuplicityVersion() then
            ...
            function GetRobberyCops() -- RETORNA A LISTA DE COM NOME, SOBRENOME, ID E CARGO DENTRO DA POLÍCIA, PARA O POLICIAL MAIOR ESCOLHER ENTRE ELES.
                local Cops = {}
                local inRobbery = IsInRobbery
                setmetatable(Cops, {__index = table})
                local polices,services = config.getUsersByPermission(cfg.PermissaoPolicial)
                for nuser_id,nsource in pairs(polices) do
                    if not inRobbery(nsource) then
                        local identity = config.getUserIdentity(nuser_id) or {}
                        local Data = vRP.DataGroups(cfg.PermissaoPolicial)
                        local Hierarchy = vRP.Hierarchy(cfg.PermissaoPolicial)
                        local cargo = Hierarchy[parseInt(Data[nuser_id])]
                        identity.name = identity.name or ''
                        identity.name2 = identity.name2 or ''
                        Cops:insert({ ['name'] = ts(identity.name).. ' '..ts(identity.name2), ['id'] = nuser_id, ['cargo'] = ts(cargo)  })
                    end
                end
            ...
            function GetRobberyBandits(source,radius) -- RETORNA A LISTA DE PESSOAS PRÓXIMAS AO LOCAL, COM NOME, SOBRENOME E ID, PARA SER ESCOLHIDOS POR QUEM PUXAR O ROUBO
                ...
                for _,v in next,Nusers do
                    if v and tn(v) then
                        local nuser_id = config.getUserId(parseInt(v))
                        if nuser_id then
                            ...
                            if not inRobbery(source) and not config.hasPermission(nuser_id, ts(cfg.PermissaoPolicial)) then
                                Bandits:insert({ name = ts(nidentity.name).. " " ..ts(nidentity.name2),  id = parseInt(nuser_id), fixed = (parseInt(v) == source)})
                            end
                        end
                    end
                end
            ...
            function GetBestCop(name) -- RETORNA O POLICIAL DE CARGO MAIS ALTO QUE ACEITOU ESCOLHER OS TIMES
                local Oficiais,Services = config.getUsersByPermission(cfg.PermissaoPolicial)
                local Hierarquizado = {}
                local inRobbery = IsInRobbery
                setmetatable(Hierarquizado, {__index = table})
                for i=1,8 do
                    for nuser_id,nsource in pairs(Oficiais) do
                        if not inRobbery(source) then
                            Hierarquizado:insert({ perm = i, user_id = nuser_id })
                            break
                        end
                    end
                end
            ...
            function AnyKillInRobbery(key,victim,reason,killer) -- FUNÇÃO PARA REGISTRAR UMA KILL NO ROUBO E SETAR AS KILLS/DEATHS NA TABELA ACTUALROBBERIES
                ...
                if killer and type(killer) == 'table' then
                    ...
                    if type(data_killer) == 'table' then 
                        ...
                        if type(kil) ~= 'string' then 
                            ...
                            kil = ts(identity.name)..' '..ts(identity.name2)
                        end 
                    end
                end
                ...
                if type(data_victim) == 'table' then 
                    ...
                    if type(vic) ~= 'string' then 
                        ...
                        vic = ts(identity.name)..' '..ts(identity.name2)
                    end 
                end
        ...
        _G["server-side/tunnel.lua"] = function()
            ...
            function tClient.tryRobbery(point) -- PRIMEIRA FUNÇÃO DE START DE ROUBO (VERIFICA O MIN DE POLICIAIS ONLINE, SE A PESSOA TEM O ITEM, SE ESTÁ EM COOLDOWN... SISTEMA DE HACK TBM)
                ...
                local id,cops = config.getUsersByPermission(cfg.PermissaoPolicial)
                if cops < min then
                ...
                return ts(obj.name),GetRobberyBandits(source,50),{ key = Rkey, proj = proj, bandlimit = GetMaxBanditsRobbery(proj,cops) }
            end
            ...
            function tClient.adminCheck(key,rcops) -- TERCEIRA FUNÇÃO, AGR COM OS POLICIAIS, ELE VERIFICA SE PRECISA DA APROVAÇÃO DO ADM, SE NÃO STARTA O ROUBO
                ...
                if cfg.NeedAdminAuthorization then
                    ...
                    CreateThread(function()
                        local id,staffs = config.getUsersByPermission(cfg.StaffPermission)
        ...
        _G["server-side/events.lua"] = function()
                                ...
                                if key == key2 then
                                    ...
                                    CreateThread(function()
                                        ...
                                        if type(victim_datatable) ~= 'table' or not victim_datatable.name or not victim_datatable.id then 
                                            ...
                                            victim_datatable = { 
                                                id = victim_id,
                                                name = tostring(victim_identity.name or '').. ' '..tostring(victim_identity.name2 or '')
                                            }
                                        end
                                        if type(assasin_datatable) ~= 'table' or not assasin_datatable.name or not assasin_datatable.id then 
                                            ...
                                            assasin_datatable = { 
                                                id = assasin_id,
                                                name = tostring(assasin_identity.name or '').. ' '..tostring(assasin_identity.name2 or '')
                                            }
                                        end        
            ...
            RegisterNetEvent("Connect", function(user_id,source)
                    ...
                    if #livePlayers > 0 then
                        ...
                        local spectatePlayer = function(data)
                            ...
                            if playerSrc then
                                ...
                                if playerPed and DoesEntityExist(playerPed) and GetEntityHealth(playerPed) > 100 and GetEntityHealth(ped) <= 100 then
                    ...
                    for k,v in next,(RobberyPoints or {}) do
                        ...
                        if distance < 10 then
                            ...
                            if distance < 1.5 then
                                ...
                                end					TimeDistance = 1                                if IsControlJustPressed(0,38) and config.isAlive(ped) and not inPainel then
