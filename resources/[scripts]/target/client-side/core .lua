...
CreateThread(function()
	...
	AddCircleZone("WorkBus",vec3(453.47,-607.76,28.58),0.5,{
		name = "WorkBus",
		heading = 3374176
	},{
		Distance = 1.25,
	...
	AddTargetModel({ -273279397, 1434516869, 1898296526, -896997473, -52638650, 591265130, 1120812170, -1748303324, -1366478936, -915224107, 10106915, 2090224559, 322493792, 1069797899},{
		options = {
			{
				event = "inventory:VerifyObjects",
				label = "Vasculhar",
	...
	AddCircleZone("tabletVehicles01",vec3(113.24,-1498.35,29.54),1.5,{
	...
	AddCircleZone("tabletVehicles02",vec3(115.73,-1521.07,29.25),0.5,{
		...
		while LocalPlayer["state"]["Target"] do
			...
			if hitZone == 1 then
				...
				for k,v in pairs(Zones) do
					...
					if #(Coords - Zones[k]["center"]) <= 10 then
						SetDrawOrigin(Zones[k]["center"]["x"],Zones[k]["center"]["y"],Zones[k]["center"]["z"])
				...
				if GetEntityType(Entity) ~= 0 then
							...
							if not Combustivel then
								if GetSelectedPedWeapon(Ped) == 883325847 then
									...
									if GlobalState["Plates"][Plate] then
										...
										Menu[#Menu + 1] = { event = "garages:Key", label = "Criar Chave Cópia", tunnel = "Policia" }
									...
									if LocalPlayer["state"]["Policia"] then
										Menu[#Menu + 1] = { event = "police:Plate", label = "Verificar Placa", tunnel = "Policia" }
										Menu[#Menu + 1] = { event = "police:Impound", label = "Registrar Veículo", tunnel = "Policia" }

										if GlobalState["Plates"][Plate] then
											Menu[#Menu + 1] = { event = "police:Arrest", label = "Apreender Veículo", tunnel = "Policia" }
										end
									else
										...
										for k,v in pairs(Tows) do
											...
											if Distance <= 10 then
												...
												Menu[#Menu + 1] = { event = "impound:Check", label = "Impound", tunnel = "Policia" }
											end
										end
									end

									Menu[#Menu + 1] = { event = "engine:vehTuning", label = "Verificar", tunnel = "client" }
								end
							else
								Selected[5] = false
								Menu[#Menu + 1] = { event = "engine:Supply", label = "Abastecer", tunnel = "client" }
					...
					elseif IsPedAPlayer(Entity) then
						if #(Coords - entCoords) <= 1.0 then
							...
							if LocalPlayer["state"]["Policia"] then
								Menu[#Menu + 1] = { event = "police:runInspect", label = "Revistar", tunnel = "Policia" }
								Menu[#Menu + 1] = { event = "police:prisonClothes", label = "Uniforme Presidiário", tunnel = "Policia" }
							elseif LocalPlayer["state"]["Hospital"] then
								if GetEntityHealth(Entity) <= 100 then
									Menu[#Menu + 1] = { event = "paramedic:Revive", label = "Reanimar", tunnel = "Hospital" }
								else
									Menu[#Menu + 1] = { event = "paramedic:Treatment", label = "Tratamento", tunnel = "Hospital" }
									Menu[#Menu + 1] = { event = "paramedic:Reposed", label = "Colocar de Repouso", tunnel = "Hospital" }
									Menu[#Menu + 1] = { event = "paramedic:Bandage", label = "Passar Ataduras", tunnel = "Hospital" }
									Menu[#Menu + 1] = { event = "paramedic:presetBurn", label = "Roupa de Queimadura", tunnel = "Hospital" }
									Menu[#Menu + 1] = { event = "paramedic:presetPlaster", label = "Colocar Gesso", tunnel = "Hospital" }
									Menu[#Menu + 1] = { event = "paramedic:extractBlood", label = "Extrair Sangue", tunnel = "Hospital" }
								end

								Menu[#Menu + 1] = { event = "paramedic:Diagnostic", label = "Informações", tunnel = "Hospital" }
								Menu[#Menu + 1] = { event = "paramedic:Bed", label = "Deitar Paciente", tunnel = "Hospital" }
							end

							if IsEntityPlayingAnim(Entity,"random@mugging3","handsup_standing_base",3) then
								Menu[#Menu + 1] = { event = "player:checkShoes", label = "Roubar Sapatos", tunnel = "Hospital" }
							end
...
RegisterNUICallback("Select",function(Data,Callback)
	...
	elseif Data["tunnel"] == "Hospital" then
