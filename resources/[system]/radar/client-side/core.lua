...
CreateThread(function()
	while true do
		...
		if IsPedInAnyPoliceVehicle(Ped) and LocalPlayer["state"]["Policia"] or LocalPlayer["state"]["Core"] then
			if policeRadar then
...
RegisterCommand("toggleFreeze", function()
	local Ped = PlayerPedId()
	if IsPedInAnyPoliceVehicle(Ped) and LocalPlayer["state"]["Policia"] or LocalPlayer["state"]["Core"] and not IsPauseMenuActive() then
