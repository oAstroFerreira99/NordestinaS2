...
-----------------------------------------------------------------------------------------------------------------------------------------
Maestria = {}
Tunnel.bindInterface("hud", Maestria)
		...
		if LocalPlayer["state"]["Active"] then
			...
			if IsPauseMenuActive() then
				...
				if Display then
					...
					if Oxygen ~= CurrentOxygen then
						SendNUIMessage({ name = "Oxygen", payload = CurrentOxygen * 10 })
			...
			if StressTimer <= GetGameTimer() then
				...
				if Stress >= 80 and GetEntityHealth(Ped) > 100 then
					...
					ApplyDamageToPed(Ped, math.random(2), false)
					TriggerEvent("Notify","thirst","Sofrendo de Stress.",2500)
